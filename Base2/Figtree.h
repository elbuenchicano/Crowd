...
///figtree wraper
///
void mat2double(cv::Mat in, double * out)
{
	if (in.channels() == 1)
	{
		int tam = in.rows * in.cols, 
			cont = 0;
		out = new double[tam];
...
/*
Mat ChangeDetectionFigtreematch(Mat &src, Mat &trgt)
{
// The dimensionality of each sample vector.
int d = trgt.cols;
...
Mat q = Mat::ones(Size(1,M),CV_64F);
double *qPtr = (double*)q.ptr();

int W = 1;

double* x = (double*)src.ptr();

double* y = (double*)trgt.ptr();

Mat prob = Mat::ones(Size(1,M),CV_64F);
double *probPtr = (double*)prob.ptr();

figtree( d, N, M, W, x, this->h, qPtr, y, this->epsilon, probPtr);

return prob;
}
/**/
void FigTree(int d, int N, int M, int W, cv::Mat &matx, double h,
	double * q, cv::Mat &maty, double epsilon, std::vector<double> & out,
	...
	int verbose = 1)
{
	...
	for (int i = 0; i < matx.rows; ++i)
	{
		...
		{
			x[cont++] = matx.at<float>(i, j);
		}
	}

	//mat2double(maty, y);

	tam		= maty.rows * maty.cols;
	cont	= 0;
	y = new double[tam];
	for (int i = 0; i < maty.rows; ++i)
	{
		...
		{
			y[cont++] = maty.at<float>(i, j);
		}
	}

	cv::Mat prob = cv::Mat::ones(cv::Size(1, M), CV_64F);
	g = (double*)prob.ptr();

	figtree(d, N, M, W, x, h, q, y, epsilon, g);
